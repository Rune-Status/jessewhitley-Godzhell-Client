...
public class DrawingArea extends Class30_Sub2
{
    ...
    public static void initDrawingArea(int i, int j, int k, int ai[], float[] depthBuffer)
    {
        pixels = ai;
        while(k >= 0) 
            anInt1374 = 275;
        width = j;
        height = i;
        setDrawingArea(i, 0, false, j, 0);
		if(settings.enableDepthBuffer)
			DrawingArea.depthBuffer = depthBuffer;
    }

    public static void defaultDrawingAreaSize(int i)
    {
        if(i < 4 || i > 4)
        {
            return;
    ...
    public static void setDrawingArea(int i, int j, boolean flag, int k, int l)
    {
        if(j < 0)
            j = 0;
        if(l < 0)
    ...
    public static void setAllPixelsToZero(boolean flag)
    {
        if(!flag)
    ...
    public static int viewport_centerY;public static int anInt1387;
